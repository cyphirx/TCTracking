...
def login():
    ...
    return render_template('auth/login.html', form=form, show_login=show_login)
...
def register():
    ...
    if form.validate_on_submit():
        ...
        #TODO create a confirmation screen for incursion herders
...
def test_login():
    ...
    resp.set_cookie('show_login', '1', max_age=30*24*60*60)
    return resp

@auth.route('/sso-login')
def sso_login():
    resp = make_response(redirect(url_for('.login')))
    resp.set_cookie('show_login', '2', max_age=30*24*60*60)
    return resp
...
def local_login():
    resp = make_response(redirect(url_for('.login')))
    resp.set_cookie('show_login', '3', max_age=30*24*60*60)
    return resp